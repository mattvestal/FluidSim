...
!want to parallelize the u and v calculation for more speed
INTEGER :: thread_id
!$OMP PARALLEL
!$OMP SINGLE PRIVATE(thread_id)
CALL uvel
WRITE(6,*)"U calc from process", thread_id
!$OMP END SINGLE

!$OMP SINGLE PRIVATE(thread_id)
CALL vvel
WRITE(6,*)"V calc from process", thread_id
!$OMP END SINGLE
!$OMP END PARALLEL
...
SUBROUTINE uvel
  ...
  REAL :: Rx(0:ny+1,0:nx+1)
  REAL :: uu, duu,speed, vu
  REAL :: pgrdx, tx
  REAL :: div, div1, div2
  REAL :: advx(0:ny+1,0:nx+1)
  REAL :: diffu, term1, term2, term3, term4, hu
  REAL :: s1, s2, h1

  DO j = 0,ny+1
  DO k = 0,nx
    CuP(j,k) = 0.25*(u(j,k)+abs(u(j,k))+u(j,k+1)+abs(u(j,k+1)))*dt/dx
    CuN(j,k) = 0.25*(u(j,k)-abs(u(j,k))+u(j,k+1)-abs(u(j,k+1)))*dt/dx
    CvP(j,k) = 0.25*(v(j,k)+abs(v(j,k))+v(j,k+1)+abs(v(j,k+1)))*dt/dy
    CvN(j,k) = 0.25*(v(j,k)-abs(v(j,k))+v(j,k+1)-abs(v(j,k+1)))*dt/dy
...
SUBROUTINE vvel
  ...
  REAL :: Ry(0:ny+1,0:nx+1)
  REAL :: vv, dvv, speed, uv
  REAL :: pgrdy, ty
  REAL :: div, div1, div2
  REAL :: advy(0:ny+1,0:nx+1)
  REAL :: diffv, term1, term2, term3, term4, hv
  REAL :: s1, s2, h1

  ! calculate the nonlinear terms for v-momentum equation
  DO j = 0,ny
  DO k = 0,nx+1
    CuP(j,k) = 0.25*(u(j,k)+abs(u(j,k))+u(j+1,k)+abs(u(j+1,k)))*dt/dx
    CuN(j,k) = 0.25*(u(j,k)-abs(u(j,k))+u(j+1,k)-abs(u(j+1,k)))*dt/dx
    CvP(j,k) = 0.25*(v(j,k)+abs(v(j,k))+v(j+1,k)+abs(v(j+1,k)))*dt/dy
    CvN(j,k) = 0.25*(v(j,k)-abs(v(j,k))+v(j+1,k)-abs(v(j+1,k)))*dt/dy
...
IF(mmode == 2)THEN
  ...
  IF(r > 0.0)THEN
    IF(r > 1.0) THEN
      psi = MIN(r, 2.0/(1.-cfl))
    ELSE
      IF(cfl > 0.0) psi = MIN(2.0*r/cfl,1.0)
    END IF
  END IF
END IF
